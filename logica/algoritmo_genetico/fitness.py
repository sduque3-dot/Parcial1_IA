"""
Módulo de función de fitness para el algoritmo genético.

Contiene funciones para evaluar el rendimiento (fitness) de individuos
del algoritmo genético midiendo qué tan efectiva es una estrategia
jugando contra diferentes tipos de oponentes.
"""
...
def convertir_resultado_a_puntos(resultado_final, mi_marca_referencia):
    """
    Convierte el resultado de una partida en una puntuación numérica para fitness.
...
def evaluar_estrategia_contra_aleatorio(estrategia_de_juego, numero_partidas=8):
    """
    Función de fitness principal: evalúa el rendimiento de un individuo del AG.
    ...
    Args:
        estrategia_de_juego (EstrategiaDeJuego): El individuo a evaluar
        numero_partidas (int): Número de rondas de evaluación (default: 8)
                              Cada ronda incluye 2 partidas (como X y como O)
    
    Returns:
        float: Fitness total acumulado a través de todas las partidas
    ...
    Note:
        ...
        - Un individuo perfecto contra oponente aleatorio debería obtener ~4-5 puntos por ronda
        - El número total de partidas jugadas es 2 * numero_partidas
    """
    # Crear instancias necesarias para la evaluación de fitness
    ...
    puntaje_acumulado = 0  # Fitness acumulado
    indice_partida = 0
    ...
    while indice_partida < numero_partidas:
        ...
        # Convertir resultados a puntuaciones de fitness y acumular
